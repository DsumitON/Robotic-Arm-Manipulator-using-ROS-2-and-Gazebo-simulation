import rclpy
from rclpy.node import Node
from arduinobot_msgs.srv import AddTwoInts
import sys


class SimpleServiceClient(Node):

    def __init__(self,a,b):
        super().__init__('simple_service_client')
        self.client_ = self.create_client(AddTwoInts, 'add_two_ints')


        while not self.client_.wait_for_service(timeout_sec=1.0):
            self.get_logger().info('service not available, waiting again...')
        self.request_= AddTwoInts.Request()


        self.request_.a = a
        self.request_.b = b
        self.future = self.client_.call_async(self.request_)

        self.future.add_done_callback(self.responsecallback)

    def responsecallback(self, future):
        self.get_logger().info('Result of add_two_ints: for %d + %d = %d' % (self.request_.a, self.request_.b, future.result().sum))
        

    

def main():
    rclpy.init()
    if len(sys.argv) != 3:
        print("Usage: ros2 run arduinobot_py_example simple_service_client A B")
        return -1
    simple_service_client = SimpleServiceClient(int(sys.argv[1]), int(sys.argv[2]))
    rclpy.spin(simple_service_client)
    simple_service_client.destroy_node()
    rclpy.shutdown()


if __name__ == '__main__':
    main()